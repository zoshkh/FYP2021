...
Returns = pd.read_csv('path toFF3_Returns.csv')
...
# we need only market factor for the CAPM thus HML and SMB are removed
del Factors['SMB']
del Factors['HML']
...
def time_series(data, names):
    ...
    for i in names:
        ...
        #print(est.summary())
       	# f = open('BM_sorted_CAPM_Time_series.tex', 'a')
...
def prepare_data_second_stage(source,first_start,first_end,second_start,second_end,third_start,third_end): 
    ...
    GRS_1.to_csv('path for BM_Sorted_CAPM_1.csv')
    GRS_2.to_csv('path for BM_Sorted_CAPM_2.csv')
    GRS_3.to_csv('path for BM_Sorted_CAPM_3.csv')
